...
func _ready():
	...
	rooms = [control_room, cafeteria, waste_room, reactor_room, turbine_room, pump_room]
	...
	if control_room.connect("mouse_entered", self, "on_room_mouse_enter") != OK:
		print("error")
	if control_room.connect("mouse_exited", self, "on_room_mouse_exit") != OK:
		print("error2")
...
func _unhandled_input(event):
	if event is InputEventMouseButton and event.button_index == BUTTON_LEFT:
		if event.pressed:
			for room in rooms:
				room = room as Room
				var rect := room.collision_shape as RectangleShape2D
				#rect. rect.extents.
		else:
			if held_object:
